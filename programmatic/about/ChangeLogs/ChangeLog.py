...
def changelog_as_text(input, nlogs):
...
#
def changelog_as_xhtml(input, nlogs):
...
if __name__ == '__main__':
  ...
  try:
    options, args = getopt.getopt(sys.argv[1:], "hf:i:n:v", ["help", "format=", "input=", "nlogs=", "version"])
  ...
  if output_format == "xhtml":
    sys.exit(changelog_as_xhtml(input, nlogs))
  elif output_format == "text":
    sys.exit(changelog_as_text(input, nlogs))
  else:
